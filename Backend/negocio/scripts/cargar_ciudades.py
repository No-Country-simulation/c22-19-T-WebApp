...
def run():
    ...
    for nombre_provincia, ciudades in datos.items():
        provincia, created = Provincia.objects.get_or_create(nombre = nombre_provincia)
        ...
        for ciudad_nombre in ciudades:
            ciudad, created = Ciudad.objects.get_or_create(nombre=ciudad_nombre, provincia=provincia)
            if created:
